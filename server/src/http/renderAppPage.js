...
function renderAppPage(selectedModule) {
...
<head>
  ...
  <title>${escapeHtml(title)} request tester</title>
  <style>
    ...
    .method { display: inline-flex; align-items: center; justify-content: center; border-radius: 999px; font-weight: 800; background: #334155; color: #e5e7eb; padding: 8px; height: 24px; }
    .method.GET { background: #166534; }
    .method.POST { background: #1d4ed8; }
    .method.PUT, .method.PATCH { background: #854d0e; }
...
<body>
  <main>
    <div class="top">
      ...
      <div class="row">
        <a href="/moduleinfo">Module info</a>
    ...
    <section class="card">
      <div class="row">
        <button class="secondary" id="open-presets">Search presets</button>
        <button id="send-request">Send request</button>
      </div>
      <p id="selected-description" class="desc">Choose a preset or enter a raw request manually.</p>
    </section>
    ...
    <section class="card">
      <div class="row">
        ...
        <div style="flex: 1">
          ...
          <input id="url" value="/" placeholder="/translate">
        </div>
      </div>
    </section>
  ...
  <div class="modal" id="preset-modal">
    <div class="modal-panel">
      <div class="row">
        <input id="preset-search" placeholder="Search by method, URL, or description">
        <button class="secondary" id="close-presets">Close</button>
      </div>
  ...
  <script>
    ...
    function presetMatches(preset, search) {
      const haystack = [preset.method, preset.path, preset.title, preset.description].join(' ').toLowerCase();
    ...
    function renderPresets() {
      ...
      const matches = state.presets.filter((preset) => presetMatches(preset, search));
      list.innerHTML = matches.length ? '' : '<p class="muted">No matching presets.</p>';
      for (const preset of matches) {
        const item = document.createElement('div');
        item.className = 'preset';
        item.innerHTML = '<div class="method ' + preset.method + '">' + preset.method + '</div>' +
          '<div><strong>' + preset.path + '</strong><p>' + (preset.description || 'No description.') + '</p></div>';
    ...
    async function loadPresets() {
      const response = await fetch('/app/presets');
      const data = await response.json();
      state.presets = data.endpoints || [];
      renderPresets();
    }

    async function sendRequest() {
      const output = byId('response');
      ...
      try {
        const payload = {
          ...
          query: parseJsonField('query'),
          body: parseJsonField('body')
        };
